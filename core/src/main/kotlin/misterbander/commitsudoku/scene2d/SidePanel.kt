...
//	}
	private lateinit var keypad: Table
...
//	}
	...
	fun build()
	{
		...
		scene2d {
			defaults().pad(5F)
			modeLabel = label(if (isEditing) "Edit Mode" else "Playing") { it.left() }
			row()
			timerLabel = label("0 : 00") { it.left() }.cell(spaceBottom = 64F)
			row()
			table {
				defaults().pad(10F).size(54F, 54F)
				editButton = imageButton(NEW_BUTTON_STYLE) {
					onChange {
						if (!isEditing)
							isEditing = true
						else
				...
				playButton = imageButton(PLAY_BUTTON_STYLE) {
					onChange {
						if (isEditing)
							isEditing = false
						else
